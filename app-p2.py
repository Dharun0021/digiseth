...
buffer_size = 2048  # Increased buffer size for smoother data flow
...
fig, ax = plt.subplots()
x = np.linspace(0, 1000, buffer_size)  # X-axis now spans from 0 to 1000
line, = ax.plot(x, audio_data_buffer)
ax.set_xlim([0, 1000])  # Set X-axis limits to 0 to 1000
ax.set_ylim([0, 1000])  # Set Y-axis limits for the range 0 to 1000
...
def update_plot():
    ...
    smoothed_data = smooth_data(audio_data_buffer)

    # Update the plot with smoothed data
    ...
    # Schedule the next update after 100ms (slower update rate)
...
root.after(100, update_plot)  # Increased update delay for smoother animation
